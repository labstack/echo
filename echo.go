...
type (
	...
	// RouteFunc contains route, handler function and middleware function.
	RouteFunc struct{
		Fn func(string, HandlerFunc, ...MiddlewareFunc)
		H HandlerFunc
		M []MiddlewareFunc
	}

	// RouteMap represents path and route function as key, value
	RouteMap map[string]RouteFunc

	// Namespace contains namespace path and route map
	Namespace struct{
		Path string
		routeMap RouteMap
	}
...
// NewNamespace register new namespace with path
func (e *Echo) NewNamespace(path string)	Namespace{
	ns := Namespace{Path: path}
	ns.routeMap = make(map[string]RouteFunc)  //set empty map at beginning
	return ns
}


// Add add path and handler function to route map of specific namespace
func(ns *Namespace) Add(path string, fn func(string, HandlerFunc, ...MiddlewareFunc), h HandlerFunc, m ...MiddlewareFunc){
	routeFunc := RouteFunc{
		Fn: fn,
		H: h,
		M: m,
	}
	ns.routeMap[path] = routeFunc
}

// Route registers new routes with namespace path and matching handler
func (ns Namespace) Route(){
	for path, routeFunc:= range ns.routeMap{
		routeFunc.Fn(ns.Path + path, routeFunc.H, routeFunc.M...)
	}
}
